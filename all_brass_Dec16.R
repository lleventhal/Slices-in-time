...
#Parameters
Vrs<-c(1:14) #variables want to calculate
...
group<-"All" #or "All"
Plot<-"Comp" #pick the batch you want
Type<-"Part"
...
if (group == "All") {
  ...
  } #end if statement over Fwest problem
  if (Plot == "B30" | Plot == "Comp"){
  ...
  if (Plot == "Fwest") {
    flwcount<-read.csv(paste("fwphenZerosJune7-2013.csv.csv", sep=""))
  ...
  if (Plot == "Fwest" | Plot == "Track"){
    BrapaP<- BrapaP %>% mutate(., batchdurG = mean(12:14)) %>% mutate(., batchdurG = mean(14:16))%>% mutate(., batchdurI = mean(16:18))%>% mutate(., batchdurJ = mean(18:20))%>% mutate(., batchdurK = mean(20:22))%>% mutate(., batchdurL = mean(22:24))%>% mutate(., batchdurM = mean(24:26))%>% mutate(., batchdurN = mean(26:28))%>% mutate(., batchdurN2 = mean(28:30))%>% mutate(., batchdurO = mean(30:31))%>% mutate(., batchdurO2 = mean(31:32))%>% mutate(., batchdurP = mean(32:33))%>% mutate(., batchdurQ = mean(33:34))
    BrapaP<- flwcount %>% mutate(., batchsumG = mean(12:14)) %>% mutate(., batchsumG = mean(14:16))%>% mutate(., batchsumI = mean(16:18))%>% mutate(., batchsumJ = mean(18:20))%>% mutate(., batchsumK = mean(20:22))%>% mutate(., batchsumL = mean(22:24))%>% mutate(., batchsumM = mean(24:26))%>% mutate(., batchsumN = mean(26:28))%>% mutate(., batchsumN2 = mean(28:30))%>% mutate(., batchsumO = mean(30:31))%>% mutate(., batchsumO2 = mean(31:32))%>% mutate(., batchsumP = mean(32:33))%>% mutate(., batchsumQ = mean(33:34))
    
    BrapaP_male<-BrapaP_male[,-c(2:10,18:40)]#need to alter this to get rid of everything exept the new varibales
  ...
  #same thing for fwest and track
  #then
  ...
  for (v in Vrs){
    ...
    if (v == 22){var22<-expression(varPed(x="z.batchdurN", gender="Male"))}
    if (v == 23){var23<-expression(varPed(x="z.batchdurN2", gender="Male"))}
    if (v == 24){var24<-expression(varPed(x="z.batchsdurO", gender="Male"))}
    if (v == 25){var25<-expression(varPed(x="z.batchdurO2", gender="Male"))}
    if (v == 26){var26<-expression(varPed(x="z.batchdurP", gender="Male"))}
    if (v == 27){var27<-expression(varPed(x="z.batchdurQ", gender="Male"))}
    if (v == 28){var28<-expression(varPed(x="z.batchsumG", gender="Male"))}
    if (v == 29){var29<-expression(varPed(x="z.batchsumH", gender="Male"))}
    if (v == 30){var30<-expression(varPed(x="z.batchsumI", gender="Male"))}
    if (v == 31){var31<-expression(varPed(x="z.batchsumJ", gender="Male"))}
    if (v == 32){var32<-expression(varPed(x="z.batchsumK", gender="Male"))}
    if (v == 33){var33<-expression(varPed(x="z.batchsumL", gender="Male"))}
    if (v == 34){var34<-expression(varPed(x="z.batchsumM", gender="Male"))}
    if (v == 35){var35<-expression(varPed(x="z.batchssumN", gender="Male"))}
    if (v == 36){var36<-expression(varPed(x="z.batchssumN2", gender="Male"))}
    if (v == 37){var37<-expression(varPed(x="z.batchssumO", gender="Male"))}
    if (v == 38){var38<-expression(varPed(x="z.batchssumO2", gender="Male"))}
    if (v == 39){var39<-expression(varPed(x="z.batchssumP", gender="Male"))}
    if (v == 40){var40<-expression(varPed(x="z.batchssumQ", gender="Male"))}
...
#PdP<-PdataPed(formula=list(res.osnotpar, res.osterr, res.phen, var1,var2, var3,var4,var5,var6,var7), data=BrapaP_for_MB)
